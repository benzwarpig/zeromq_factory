...
public:
    ...
    zmq::socket_t& GetSocketRef() { return socket; }
...
private:
    ValueZeroMqPublish() { spdlog::info("ValueZeroMqPublish is Created"); };
    ~ValueZeroMqPublish() { spdlog::info("ValueZeroMqPublish is Destory"); };
...
public:
    ...
    void RegisterZMQNode(const std::string& ip) {
        ...
        zeromq_pack.emplace(
            std::make_pair(MsgType().GetTypeName(), std::move(tmp)));
    }
    ...
    void PublishProtoMsg(const MsgType& msg) {
        ...
        if (msg.SerializeToString(&tmp)) {
            if (0 != zeromq_pack.count(msg.GetTypeName())) {
                zeromq_pack[msg.GetTypeName()]->socket.send(
                    zmq::const_buffer(tmp.data(), tmp.size()));
            } else {
                spdlog::error("without the member of this pack , {}",
                              msg.GetTypeName());
            }
        }
    }
...
private:
    ValueZeroMqSubscribe() : stop_flag(false) {
        spdlog::info("ValueZeroMqSubscribe is Created");
        subscribe_thread = std::make_shared<std::thread>(
            &ValueZeroMqSubscribe::ZeroMqSubscribeThread, this);
    };
...
public:
    ...
    void RegisterZMQNode(const std::string& ip,
        ...
        std::shared_ptr<ZeroMqImpl> tmp_zeromq_impl =
            std::make_shared<ZeroMqImpl>(ZMQ_SUB);
        ...
        std::shared_ptr<google::protobuf::Message> tmp_proto =
            std::make_shared<MsgType>();

        subscribe_pack.emplace(std::make_pair(
            MsgType().GetTypeName(),
            std::make_tuple(std::move(tmp_zeromq_impl), std::move(tmp_proto),
                            std::move(callback))));
    }
...
private:
    ...
    std::unordered_map<std::string,
                       std::tuple<std::shared_ptr<ZeroMqImpl>,
...
private:
    ValueZeroMqServce() : stop_flag(false) {
        servce_thread = std::make_shared<std::thread>(
            &ValueZeroMqServce::ZeroMqServceThread, this);
...
public:
    template <typename MsgType>
    void RegisterZMQNode(
        const std::string& ip,
        std::function<void(void*, const zmq::socket_t&)>&& callback) {
        std::shared_ptr<ZeroMqImpl> tmp_node =
            std::make_shared<ZeroMqImpl>(ZMQ_REP);
        ...
        servce_pack.emplace(std::make_pair(
            MsgType().GetTypeName(),
            std::make_tuple(std::move(tmp_node), std::move(tmp_proto),
                            std::move(callback))));
    }
            ...
            if (servce_pack.size() != 0) {
                ...
                for (const auto& tmp : servce_pack) {
                    ...
                    if (zeromq_tmp->GetSocketRef().recv(str_data)) {
                        if (proto_tmp->ParseFromString(str_data.to_string())) {
                            callback(dynamic_cast<void*>(proto_tmp.get()),
                                     zeromq_tmp->GetSocketRef());
                        } else {
                            spdlog::error("{} parse failure!!!!",
                                          proto_tmp->GetTypeName());
                            std::string temp = "failure";
                            zeromq_tmp->GetSocketRef().send(
                                zmq::const_buffer(temp.data(), temp.size()));
                        }
                    }
                }
            }
...
private:
    ...
    std::unordered_map<
        ...
        std::tuple<std::shared_ptr<ZeroMqImpl>,
                   std::shared_ptr<google::protobuf::Message>,
                   std::function<void(void*, const zmq::socket_t&)> > >
        servce_pack;
};  // ValueZeroMqServce
...
public:
    ...
    void RegisterZMQNode(const std::string& ip) {
        ...
        int timeout = 1000;
        tmp->socket.set_option(
            ZMQ_RCVTIMEO, &timeout,
            sizeof(timeout));  // 在一个recv操作返回EAGAIN错误前的最大时间
        tmp->socket.set_option(
            ZMQ_SNDTIMEO, &timeout,
            sizeof(timeout));  // 在一个发送操作返回EAGAIN之前等待的最大时间

        int linger = 0;
        tmp->socket.set_option(
            ZMQ_LINGER, &linger,
            sizeof(linger));  // socket关闭时等待send内容的等待时间
