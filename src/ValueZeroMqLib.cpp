...
void SubscribeCallBack<LSLAM::MyOccupancyGrid>(void* msg) {
    ...
    spdlog::info("saved_origin_x : {},saved_origin_y : {}",
                 mmsg->saved_origin_x_(), mmsg->saved_origin_y_());
}
...
void SubscribeCallBack<LSLAM::RobotPose>(void* msg) {
    ...
    spdlog::info("x : {},y : {} ,theta: {}", mmsg->x(), mmsg->y(),
                 mmsg->theta());
}
...
int main(void) {
    ...
    ValueZeroMqPublish::GetInstance().RegisterZMQNode<LSLAM::MyOccupancyGrid>(
        "tcp://*:1234");
    ValueZeroMqPublish::GetInstance().RegisterZMQNode<LSLAM::RobotPose>(
        "tcp://*:1235");

    /* 创建一个订阅LSLAM::MyOccupancyGrid消息类型的消费者 */
    ValueZeroMqSubscribe::GetInstance().RegisterZMQNode<LSLAM::MyOccupancyGrid>(
        "tcp://localhost:1234",
        std::move(SubscribeCallBack<LSLAM::MyOccupancyGrid>));
    ValueZeroMqSubscribe::GetInstance().RegisterZMQNode<LSLAM::RobotPose>(
        "tcp://localhost:1235", std::move(SubscribeCallBack<LSLAM::RobotPose>));
